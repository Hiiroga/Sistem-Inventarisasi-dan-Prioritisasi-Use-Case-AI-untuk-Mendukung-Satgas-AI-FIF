...
<div class="bg-white rounded-3xl border border-slate-100 shadow-xs p-6">
    <h1 class="text-xl font-extrabold text-slate-800 mb-5">Edit Use Case — {{ $useCase->kode }}</h1>
    ...
    <form action="{{ route('use-cases.update', $useCase) }}" method="POST" class="space-y-6">
        @csrf @method('PUT')
        @include('use-cases._form')
        <div class="flex gap-2 pt-2">
            <button type="submit" class="px-5 py-2.5 bg-telkom-red hover:bg-red-700 text-white rounded-xl text-sm font-bold shadow-md transition-all">Update</button>
